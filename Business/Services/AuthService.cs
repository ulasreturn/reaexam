...
public class AuthService : IAuthService
{
    ...
    public async Task<DataResult<Utilities.Security.Auth.Jwt.Token>> Register(RegisterDto registerDto)
    {
        ...
        if (await _unitOfWork.User.FirstOrDefaultAsync(u => u.UserName == registerDto.UserName) != null)
            return new DataResult<Utilities.Security.Auth.Jwt.Token>(message: Messages.UserNameAlreadyTaken,
                status: ResultStatus.Invalid);

        if (await _unitOfWork.User.FirstOrDefaultAsync(u => u.Email == registerDto.Email) != null)
    ...
    public async Task<DataResult<Utilities.Security.Auth.Jwt.Token>> Login(LoginDto loginDto)
    {
        ...
        var user = await _unitOfWork.User.FirstOrDefaultAsync(x => x.Email == loginDto.Email);

        if (user == null || !_hashingHelper.VerifyPasswordHash(loginDto.PasswordSalt, user.PasswordHash, user.PasswordSalt))
        {
    ...
    public async Task<DataResult<UserInfoDto>> GetUserProfileInfo()
    {
        var userId = _claimHelper.GetUserId();

        var user = await _unitOfWork.User.FirstOrDefaultAsync(u => u.Id == userId);

        var profileDto = _mapperHelper.Map<UserInfoDto>(user);

        return new DataResult<UserInfoDto>(data: profileDto, status: ResultStatus.Ok);
    }
